...
def tube_masking(x, mask_ratio, T):
    N, L, D = x.shape  # 128*576*128
    x = x.reshape(N, T, L//T, D)  # 128*6*96*128
    N, T, L, C = x.shape

    len_keep = int(L * (1 - mask_ratio))  # 96*(1-0.75)

    noise = torch.rand(1, L, device=x.device)  # noise in [0, 1]
    noise = noise.repeat(N,1) # 128*96
    # 生成96个噪声，获得从小到大的元素的index
    ids_shuffle = torch.argsort(
        noise, dim=1
    )  # ascend: small is keep, large is remove
    ids_restore = torch.argsort(ids_shuffle, dim=1)

    # keep the first subset 保留的ids
    ids_keep = ids_shuffle[:, :len_keep]
    x_masked = torch.gather(x, dim=2, index=ids_keep.unsqueeze(1).unsqueeze(-1).repeat(1, T, 1, D))  # 128*6*24*128
    ...
    ids_keep = ids_keep.unsqueeze(1).repeat(1,T,1).reshape(N,-1)  # 128*144 (6*24)
    x_masked = x_masked.reshape(N, -1, x_masked.shape[-1])  # 128*144*128

    return x_masked, mask, ids_restore, ids_keep

def tube_block_masking(x, mask_ratio, T):
    N, L, D = x.shape  # 128*576*128
    x = x.reshape(N, T, L//T, D) # 128*6*16*128
    N, T, L, C = x.shape

    assert mask_ratio in [0.25,0.5,0.75]

    len_keep = int(L * (1 - mask_ratio))  # 保留的长度为72

    index = np.random.randint(low=0,high=4)  # 随机产生一个0-3的整数

    noise = torch.zeros(N, L, device=x.device)

    if mask_ratio  == 0.25:
        noise[:,index*L//4:(index+1)*L//4] = 1  # 0-24都遮蔽（前1/4整体遮蔽方案）
...
def antenna_masking(x, mask_ratio, T, H, W):
    N, L, D = x.shape # batch, length, dim
    x = x.reshape(N, T, H, W, D)

    len_keep = int(H * (1 - mask_ratio))


    # noise = torch.arange(H).unsqueeze(dim=0).unsqueeze(0).repeat(N,T,1) # N*T*H
    # noise = noise.to(x)  # 转到设备上
    noise = torch.rand(N, H, device=x.device).unsqueeze(dim=1).repeat(1,T,1)  # 类似于frame

    ids_shuffle = torch.argsort(
        noise, dim=2
    )  # ascend: small is keep, large is remove N*T
    ids_restore = torch.argsort(ids_shuffle, dim=2)  # N*T*H*W

    # keep the first subset
    ids_keep = ids_shuffle[:, :, :len_keep]  # N,T,H/2
    x_masked = torch.gather(x, dim=2, index=ids_keep.unsqueeze(3).unsqueeze(-1).repeat(1, 1, 1, W, D))  #
    ...
    # unshuffle to get the binary mask
    mask = torch.gather(mask, dim=2, index=ids_restore.unsqueeze(3).repeat(1,1,1,W)).reshape(N,-1)  #

    ids_keep = ids_keep.unsqueeze(-1).repeat(1,1,1,W).reshape(N,-1)
    x_masked = x_masked.reshape(N, -1, x_masked.shape[-1])
...
def antenna_restore(x, ids_restore, N, T, H, W, C, mask_token):
    # ids_restore: [N,T,H]
    x = x.reshape(N, T, -1, W, x.shape[-1])
    mask_tokens = mask_token.repeat(N, T, H-x.shape[2], W,1)
    x_ = torch.cat([x, mask_tokens],dim=2)
    x_ = torch.gather(x_, dim=2, index=ids_restore.unsqueeze(3).unsqueeze(-1).repeat(1,1,1,W,x_.shape[-1]))
    x = x_.view([N, T*H*W, C])
    return x
